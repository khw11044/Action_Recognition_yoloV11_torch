...
input_size = 34 # (17*2)
X = np.array(sequences).reshape(-1, 15, 17, 3)                     # X.shape -> (400, 15, 51)
X = X[:, :, :, :2].reshape(-1, 15, input_size)  
...
def train_and_validate(model, train_loader, val_loader, criterion, optimizer, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(model.state_dict(), './models/best_kp.pth')
...
plt.savefig('training_curve_best_01kp.png')
plt.show()

# 테스트 데이터셋 평가 함수
def evaluate_model(model, test_loader):
    model.load_state_dict(torch.load('./models/best_kp.pth'))
    model.eval()
    correct = 0
    with torch.no_grad():
